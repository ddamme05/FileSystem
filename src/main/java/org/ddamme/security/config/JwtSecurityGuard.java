...
/**
 * Security guard to prevent production deployment with default JWT secrets.
 * Fails fast at startup if default/weak secrets are detected in production.
 */
@Slf4j
@Component
    ...
    void validateJwtSecret() {
        if (environment.matchesProfiles("prod")) {
            ...
            if (jwtSecret.length() < 32) {
                log.warn("JWT secret appears short for production use. Consider using a longer base64-encoded secret.");
            }

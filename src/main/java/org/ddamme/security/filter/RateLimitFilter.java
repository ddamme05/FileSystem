...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (principalKey != null && props.getExemptPrincipals() != null && 
            props.getExemptPrincipals().contains(principalKey)) {
            filterChain.doFilter(request, response);
            return;
        }

        int perMinute = props.getPerMinute() == null ? 0 :
                props.getPerMinute().getOrDefault(routeKey, 0);
        if (perMinute <= 0) {
            filterChain.doFilter(request, response);
            return;
        }
    ...
    private String matchRouteKey(String method, String path) {
        boolean isUpload = "POST".equals(method) && 
                (path.equals("/files/upload") || path.equals("/api/v1/files/upload"));
        boolean isDownload = ("GET".equals(method) || "HEAD".equals(method)) &&
                (path.startsWith("/files/download/") || path.startsWith("/api/v1/files/download/"));
    ...
    private void incrementRateLimitCounter(String routeKey) {
        if ("upload".equals(routeKey)) {
            if (uploadRateLimitCounter == null) {
                uploadRateLimitCounter = Counter.builder("http.ratelimit.rejects")
                    .tag("route", "upload")
            ...
            if (downloadRateLimitCounter == null) {
                downloadRateLimitCounter = Counter.builder("http.ratelimit.rejects")
                    .tag("route", "download")
                    .register(meterRegistry);
            }
